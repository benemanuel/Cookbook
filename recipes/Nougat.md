---
schema_version: 1
title: Nougat
url: null
source: null
image: null
servings: null
prep_minutes: null
cook_minutes: null
imported_at: 2026-05-31
ingredients:
  - 1 cup granulated sugar
  - 1/2 cup honey
  - 2 egg whites
  - 1 cup nuts (such as almonds, hazelnuts, or pistachios), coarsely chopped
  - 1 tsp vanilla extract
steps:
  - In a small saucepan, combine the sugar and honey and cook over medium heat, stirring constantly, until the sugar has dissolved and the mixture has reached a temperature of 240°F on a candy thermometer.
  - Beat the egg whites in a large mixing bowl until stiff peaks form.
  - When the sugar mixture has reached the correct temperature, carefully pour it into the egg whites in a thin stream while continuing to beat the mixture on high speed.
  - Add the vanilla extract and nuts to the mixture and continue beating until the mixture is thick and glossy.
  - Spread the mixture evenly into a greased 8x8-inch baking dish and let the nougat cool completely before cutting it into squares.
  - Serve the nougat as is, or use it as a filling for chocolate bars or other sweets.
labels: [candy]
---

# Nougat

## Ingredients

- 1 cup granulated sugar
- 1/2 cup honey
- 2 egg whites
- 1 cup nuts (such as almonds, hazelnuts, or pistachios), coarsely chopped
- 1 tsp vanilla extract

## Instructions

1. In a small saucepan, combine the sugar and honey and cook over medium heat, stirring constantly, until the sugar has dissolved and the mixture has reached a temperature of 240°F on a candy thermometer.
2. Beat the egg whites in a large mixing bowl until stiff peaks form.
3. When the sugar mixture has reached the correct temperature, carefully pour it into the egg whites in a thin stream while continuing to beat the mixture on high speed.
4. Add the vanilla extract and nuts to the mixture and continue beating until the mixture is thick and glossy.
5. Spread the mixture evenly into a greased 8x8-inch baking dish and let the nougat cool completely before cutting it into squares.
6. Serve the nougat as is, or use it as a filling for chocolate bars or other sweets.
